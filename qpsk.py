...
sym = [0, 1, 3, 5, 6, 7]#, 8] # symbols 3, 5 intentionally left out
...
class Decoder:
    ...
    def magic(self):
        ...
        if len(np.concatenate((bits[0:]))) > 7200:
            print("FOUND NO LEGACY DRONE "+str(len(np.concatenate((bits[0:])))))
            ...
            print("FOUND LEGACY DRONE "+str(len(np.concatenate((bits[0:])))))
        ...
        print(f"First 20 bit decoded: {plo[offset:offset+20]}")
        return ba.tobytes()
